...
const SupplyManagementPanel = ({
  ...
  const scrollToSupply = (index: number) => {
  ...
  return (
    <>
      <div className="bg-white rounded-lg shadow-lg sm:rounded-2xl">
        ...
        <div className="bg-gradient-to-r from-orange-600 to-red-600 px-3 py-3 rounded-t-lg sm:px-6 sm:py-4 sm:rounded-t-2xl">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:gap-3">
              ...
              <div className="text-orange-100 w-fit text-xs bg-orange-500 bg-opacity-30 px-2 py-1 rounded-full sm:text-sm">
        ...
        {/* Supply Carousel Navigation - Mobile First */}
        <div className="p-3 sm:p-6">
          {/* Emoji Pagination */}
          <div className="flex gap-2 justify-center mb-4 sm:hidden">
            {editingSupplies.map((supply, index) => (
              <button
                key={supply.id}
                onClick={() => scrollToSupply(index)}
          ...
          {/* Desktop: Show all supplies indicator */}
          <div className="hidden sm:block mb-4">
            <p className="text-sm text-slate-600">
              Managing {editingSupplies.length} supply types
            </p>
          </div>

          {/* Supply Cards Carousel */}
          <div className="relative">
            <div
              ref={scrollRef}
              className="flex overflow-x-auto snap-x snap-mandatory gap-4 pb-4 scrollbar-hide sm:grid sm:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 sm:overflow-visible sm:gap-6"
              onScroll={(e) => {
                // Update current supply on scroll (mobile only)
                if (window.innerWidth < 640) {
                  const scrollLeft = e.currentTarget.scrollLeft;
                  const cardWidth = e.currentTarget.offsetWidth;
                  const newIndex = Math.round(scrollLeft / cardWidth);
                  setCurrentSupply(
                    Math.max(0, Math.min(newIndex, editingSupplies.length - 1))
                  );
                }
              }}
            >
              {editingSupplies.map((supply) => {
                ...
                return (
                  <div
                    key={supply.id}
                    className={`flex-shrink-0 w-full snap-start p-3 rounded-lg border transition-all sm:w-auto sm:p-5 sm:rounded-xl sm:border-2 ${styles.bgColor}`}
                  >
                    {/* Header - Mobile First */}
                      ...
                      <div className="flex items-center justify-between sm:flex-col sm:items-end sm:gap-3">
                        ...
                        >
                          ...
                          <span className="sm:hidden">
                            {weeklyStatus.status.toUpperCase().slice(0, 4)}
                          </span>
                          <span className="hidden sm:inline">
                    ...
                    weeklyStatus.status === "low" ? (
                      <div className="bg-red-50 border border-red-200 rounded-lg p-3">
                        ...
                        <p className="text-red-700 text-sm">
                          ...
                          <span className="hidden sm:inline">
                            ...
                            to reach 1 week supply. Contact donors immediately
                            or consider emergency procurement.
                          </span>
                        </p>
                      </div>
                      ...
                      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                        ...
                        <p className="text-blue-700 text-sm">
                          ...
                          <span className="hidden sm:inline">
                            ...
                            more than needed. Consider redirecting new donations
                            to other supplies or locations.
                          </span>
                        </p>
                      </div>
                    ) : (
                      <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                        ...
                        <p className="text-green-700 text-sm">
                          ...
                          <span className="hidden sm:inline">
                            Good supply level. Monitor daily consumption and
